...
class LivreurService:
    ...
    def authentifier_livreur(db: Session, contact: str, mot_de_passe: str) -> LivreurModel | None:
        try:
            print(f"Tentative d'authentification pour: {contact}")
            livreur = db.query(LivreurModel).filter(LivreurModel.contact == contact).first()

            if not livreur:
                print(f"Aucun livreur trouvé avec le contact: {contact}")
                # Ne pas créer de notification si l'utilisateur n'existe pas
                # Car on ne peut pas utiliser user_id=None
                return None

            if not livreur.mot_de_passe or not bcrypt.checkpw(mot_de_passe.encode('utf-8'), livreur.mot_de_passe.encode('utf-8')):
                print(f"Mot de passe incorrect pour le livreur: {contact}")
                # Créer notification avec l'ID du livreur existant
                try:
                    notif = NotificationCreate(
                        user_id=livreur.id,
                        user_type="livreur",
            ...
            print(f"Authentification réussie pour: {contact}")
            # Connexion réussie
            try:
                notif = NotificationCreate(
                    user_id=livreur.id,
            ...
            print(f"Erreur dans authentifier_livreur: {str(e)}")
            return None
